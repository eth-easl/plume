//===----------------------------------------------------------------------===//
// plume — Thrift compact-protocol reader (shared by the footer parser and the
// page-header parser). Just enough of the protocol to walk parquet metadata and
// page headers; unknown fields are skipped generically.
//===----------------------------------------------------------------------===//
...
public:
	...
	// Read a struct field header. Returns false on STOP; otherwise sets type/id
	// (id tracked via the running `last_id`).
...
// Thrift compact-protocol writer (the mirror of CompactReader), used by the parquet
// encoder to emit page headers and the FileMetaData footer. Field ids are
// delta-encoded within the current struct scope; nested structs (StructField /
// Element) save and restore the running id. Lists carry only a header; their
// elements are written directly (struct elements via Element{Begin,End}, scalar
// elements via the I32Elem/StringElem helpers).
class CompactWriter {
public:
	...
	// Scalar fields (absolute field id within the current struct).
	...
	// Nested struct held in a field, vs. a struct element inside a list.
	...
	// List header (followed by `n` elements written directly).
