//===----------------------------------------------------------------------===//
// plume — parquet column-chunk decoder (TASK_v2 plume_pq_stage)
//
// Scope: UNCOMPRESSED or SNAPPY chunks, Data Page V1 and V2, PLAIN and dictionary
// (PLAIN_DICTIONARY / RLE_DICTIONARY) encodings, flat columns (no repetition
// levels). Definition levels (RLE) handle nullable columns. Column types:
// bool, int8/16/32/64, float, double, varchar, date, time, and decimal backed by
// parquet INT32 (width<=9) / INT64 (width<=18). (int8/16/date/small-decimal are
// stored as parquet INT32; time/larger-decimal as INT64.)
//===----------------------------------------------------------------------===//
