...
Result<CompiledComposition> CompileQuery(duckdb::Connection &con, SourceCatalog &catalog, const std::string &sql,
    ...
    RemoteResolver resolver(AutoFetcher(), 10); // TODO: make num_threads configurable
    ...
    for (const auto &ti : out.composition.table_inputs) {
        ...
        TRY(auto materialized, MaterializeTable(con, *table_source, *ti.projection, ti.pushed_filter.get()));
    ...
    for (const auto &ri : out.composition.remote_inputs) {
        if (ri.source->type == DataSourceType::REMOTE_PARQUET) {
            ...
            TRY(auto pq_stage_inputs, BuildParquetStageInputs(*pq_src, *ri.projection, ri.pushed_filter.get(),
            ...
            TRY(auto csv_stage_inputs,
                BuildCSVStageInputs(*csv_src, *ri.projection, ri.target_splits, config.max_region_bytes));
